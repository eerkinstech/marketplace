...
export default function AdminShippingPage() {
  const { token, error, setError } = useAccessToken("Login with an admin account to manage shipping.");
  const [data, setData] = useState({ areas: [], products: [], vendorSummary: [], vendorShippingAreas: [] });
  ...
  async function load() {
    ...
    try {
      setLoading(true);
      const response = await marketplaceApi.getAdminShipping(token);
      setData(response.data || { areas: [], products: [], vendorSummary: [], vendorShippingAreas: [] });
    } catch (err) {
  ...
  async function saveArea(event) {
    ...
    try {
      ...
      if (editingAreaId) {
        await marketplaceApi.updateAdminShippingArea(token, editingAreaId, payload);
        setNotice("Shipping setup updated.");
      } else {
        await marketplaceApi.createAdminShippingArea(token, payload);
  ...
  async function removeArea(id) {
    try {
      await marketplaceApi.deleteAdminShippingArea(token, id);
    ...
    try {
      ...
      await Promise.all(
        selectedProductIds.map((productId) =>
          marketplaceApi.assignAdminShippingAreas(token, productId, { shippingAreaIds: selectedAreaIds })
        )
      );
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-sm font-semibold text-slate-600">Rules</p><p className="mt-2 text-3xl font-bold text-blue-600">{stats.totalRules}</p></div>
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm"><p className="text-sm font-semibold text-slate-600">Free Shipping Rules</p><p className="mt-2 text-3xl font-bold text-emerald-600">{stats.freeRules}</p></div>
          ...
          <div className="grid gap-6 xl:grid-cols-[1fr_1.05fr]">
            <form onSubmit={saveArea} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 grid gap-4">
                <input value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder="Shipping name" className="rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                <input value={form.estimatedDays} onChange={(event) => setForm((current) => ({ ...current, estimatedDays: event.target.value }))} placeholder="ETA, for example 3-5 business days" className="rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                    ...
                    {form.rules.map((rule, index) => (
                      <div key={`rule-${index}`} className="rounded-xl border border-slate-200 bg-white p-3">
                        <div className="grid gap-3 lg:grid-cols-2 xl:grid-cols-2">
                          <input type="number" min="0" step="0.01" value={rule.minWeight} onChange={(event) => updateRule(index, "minWeight", event.target.value)} placeholder="Min weight (kg)" className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                          <input type="number" min="0" step="0.01" value={rule.maxWeight} onChange={(event) => updateRule(index, "maxWeight", event.target.value)} placeholder="Max weight (kg)" className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                          <input type="number" min="0" step="0.01" value={rule.minPrice} onChange={(event) => updateRule(index, "minPrice", event.target.value)} placeholder="Min price" className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                          <input type="number" min="0" step="0.01" value={rule.maxPrice} onChange={(event) => updateRule(index, "maxPrice", event.target.value)} placeholder="Max price" className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" />
                          <input type="number" min="0" step="0.01" value={rule.rate} onChange={(event) => updateRule(index, "rate", event.target.value)} placeholder="Rate" disabled={rule.isFreeShipping} className="rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:bg-slate-100" />
                        </div>
                ...
                <button type="submit" disabled={saving} className="rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white transition hover:bg-slate-700 disabled:opacity-60">
                  ...
                  <div key={area._id} className="rounded-xl border border-slate-200 p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">Assigned admin products: {area.assignedProducts || 0}</p>
                      </div>
          ...
          <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-2xl font-bold text-slate-900">Vendor Shipping Overview</h2>
              <p className="mt-1 text-sm text-slate-500">Admin can review every vendor shipping setup here. Vendor shipping stays separate from admin products.</p>
              <div className="mt-5 overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 text-left text-slate-500">
                      <th className="py-3 pr-4">Vendor</th>
                      <th className="py-3 px-4">Shipping Names</th>
                      <th className="py-3 px-4">Products With Shipping</th>
                      <th className="py-3 pl-4">Total Products</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.vendorSummary.length ? data.vendorSummary.map((vendor) => (
                      <tr key={vendor._id} className="border-b border-slate-100 last:border-b-0">
                        <td className="py-4 pr-4 font-semibold text-slate-900">{vendor.vendorName}</td>
                        <td className="py-4 px-4 text-slate-600">{vendor.assignedShippingAreas || 0} assigned of {vendor.shippingAreas || 0}</td>
                        <td className="py-4 px-4 text-slate-600">{vendor.assignedProductCount || 0}</td>
                        <td className="py-4 pl-4 text-slate-600">{vendor.productCount || 0}</td>
                      </tr>
                    )) : (
                      <tr><td colSpan="4" className="py-10 text-center text-slate-500">No vendors found.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-2xl font-bold text-slate-900">Vendor Shipping Rules</h2>
              <p className="mt-1 text-sm text-slate-500">Each row shows a vendor shipping name and how many vendor products use it.</p>
              <div className="mt-5 space-y-4">
                {data.vendorShippingAreas.length ? data.vendorShippingAreas.map((area) => (
                  <div key={area._id} className="rounded-xl border border-slate-200 p-5">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="text-lg font-bold text-slate-900">{area.name}</h3>
                        <p className="mt-1 text-sm text-slate-500">Vendor: {area.vendor?.vendorName || "Unknown vendor"}</p>
                        <p className="mt-1 text-sm text-slate-500">ETA: {area.estimatedDays || "N/A"}</p>
                        <p className="mt-1 text-sm text-slate-500">Assigned vendor products: {area.assignedProducts || 0}</p>
                      </div>
                      <span className={`rounded-full px-3 py-1 text-xs font-semibold ${area.isActive ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-600"}`}>
                        {area.isActive ? "Active" : "Inactive"}
                      </span>
                    ...
                    <div className="mt-4 space-y-2 text-sm text-slate-600">
                      {(area.rules || []).map((rule, index) => (
                        <div key={`${area._id}-vendor-rule-${index}`} className="rounded-lg bg-slate-50 px-3 py-2">
                          {formatRule(rule)}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="rounded-xl border border-dashed border-slate-300 p-6 text-sm text-slate-500">No vendor shipping rules found.</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Assign only admin shipping names to admin-owned products.</p>
              </div>
              <button type="button" onClick={bulkAssign} disabled={assigning} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:opacity-60">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.products.length ? data.products.map((product) => (
                    ...
                    <tr><td colSpan="7" className="py-10 text-center text-slate-500">No admin-managed products found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
