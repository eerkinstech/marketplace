      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-[28px] border border-black/10 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.2)]">
        ...
        <form onSubmit={onSubmit} className="grid gap-5 px-6 py-6">
          ...
          <label className="grid gap-2 relative">
            <span className="text-sm font-semibold text-ink">Assign Product (Optional)</span>
            <div
            ...
            >
              <div className="px-4 py-2">
                {selectedProduct?.title || selectedProduct?.name ? (
                  ...
                  <span className="text-slate-500">Select a product or leave blank for all products</span>
                )}
              </div>
            </div>
...
export default function AdminCouponsPage() {
  const { token, error, setError } = useAccessToken("Login with an admin account to manage coupons.");
  ...
  async function loadCoupons() {
    ...
    try {
      setLoading(true);
      const response = await marketplaceApi.getAdminCoupons(token);
  ...
  const metrics = useMemo(() => {
    ...
    return [
      { label: "Coupons", value: coupons.length, detail: "All created coupon codes" },
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (editingCoupon?._id) {
        await marketplaceApi.updateCoupon(token, editingCoupon._id, payload);
      } else {
        await marketplaceApi.createCoupon(token, payload);
      }
  ...
  async function handleDelete(coupon) {
    ...
    try {
      ...
      setError("");
      await marketplaceApi.deleteCoupon(token, coupon._id);
      await loadCoupons();
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    <section className="container page-section stack">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="muted-copy mt-3">Create and manage discount codes with product assignment and usage tracking.</p>
        </div>
          ...
          <table className="w-full  text-sm">
            <thead className="border-b border-black/8 bg-slate-50/90">
              <tr>
                ...
                <th className="px-4 py-4 text-left text-xs font-semibold uppercase tracking-[0.16em] text-slate-600">Assigned Products</th>
                  ...
                  return (
                    <tr key={coupon._id} className="border-b border-black/6 transition hover:bg-slate-50/70 last:border-b-0">
                      ...
                      <td className="px-4 py-4 text-slate-700">
                        {productName ? productName : <span className="text-xs italic text-slate-500">All products</span>}
                      </td>
