...
function buildPayloadFromRow(row, categories, rowNumber) {
  ...
  return {
    ...
    status: String(getRowValue(row, ["status"]) || "").trim().toLowerCase(),
    rejectionReason: String(getRowValue(row, ["rejectionReason"]) || "").trim(),
    seo: {
...
export default function AdminProductsPage() {
  const { token, error, setError } = useAccessToken("Login with an admin account to manage products.");
  ...
  async function load() {
    if (!token) return;
    try {
      const response = await marketplaceApi.getAdminProducts(token);
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      const matchesSearch = keyword
        ? [product.name, product.sku, product.vendorLabel, product.category?.name, product.categorySlug]
          .filter(Boolean)
  ...
  async function setStatus(id, status) {
    try {
      await marketplaceApi.updateProductStatus(token, id, { status });
      setNotice(`Product marked as ${status}.`);
      await load();
    } catch (err) {
      setError(err.message);
    }
  ...
  async function removeAdminProduct(id) {
    try {
      await marketplaceApi.deleteAdminProduct(token, id);
  ...
  async function runBulkAction(action) {
    ...
    try {
      ...
      if (action === "delete") {
        await Promise.all(selectedProducts.map((product) => marketplaceApi.deleteAdminProduct(token, product._id)));
        setNotice(`${selectedProducts.length} products deleted.`);
      } else {
        await Promise.all(selectedProducts.map((product) => marketplaceApi.updateProductStatus(token, product._id, { status: action })));
        setNotice(`${selectedProducts.length} products updated to ${action}.`);
      }
      setSelectedIds([]);
      await load();
    } catch (err) {
  ...
  function exportProducts() {
    ...
    const headers = [
      ...
      "soldCount", "isFeatured", "vendorId", "vendorLabel", "createdAt", "updatedAt"
    ];
    const csv = [
      headers.join(","),
      ...rows.map((product) =>
    ...
    downloadFile("admin-products-export.csv", csv, "text/csv;charset=utf-8");
  ...
  async function handleImport(event) {
    ...
    try {
      ...
      const [text, categoriesResponse] = await Promise.all([file.text(), marketplaceApi.getAdminCategories(token)]);
      const payloads = parseImportFile(text, file.name, categoriesResponse.data || []);
      for (const payload of payloads) {
        const response = await marketplaceApi.createAdminProduct(token, payload);
        const status = String(payload.status || "").trim();
        if (response?.data?._id && ["pending", "approved", "rejected"].includes(status)) {
          await marketplaceApi.updateProductStatus(token, response.data._id, {
            status,
      ...
      <div className="rounded-[28px] border border-[#d9c9b7] bg-[linear-gradient(180deg,#fffdfa_0%,#fff7ef_100%)] p-6 shadow-[0_24px_60px_rgba(120,53,15,0.08)] md:p-8">
        <div className="flex justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-black tracking-[-0.04em] text-[#1f2937] md:text-3xl">Product Management</h1>

          </div>
          <div className="flex flex-row gap-3">
            <button className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#b56d2d] px-4 py-3 text-[13px] font-semibold text-white shadow-[0_10px_30px_rgba(181,109,45,0.25)] transition hover:-translate-y-0.5" type="button" onClick={() => fileInputRef.current?.click()} disabled={busyAction === "import"}>
              <Icon name="upload" className="h-4 w-4" />
              {busyAction === "import" ? "Importing..." : "Import Products"}
            </button>
            <Link className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#2f3136] px-4 py-3 text-[13px] font-semibold text-white transition hover:-translate-y-0.5" href="/admin/products/new"><Icon name="add" className="h-4 w-4" />Add Product</Link>
      ...
      <div className="grid gap-4 rounded-[28px] border border-black/5 bg-white/85 p-5 shadow-[0_22px_50px_rgba(15,23,42,0.06)] md:p-6">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-wrap gap-3">
            {statusTabs.map((tab) => (
              <button key={tab.id} type="button" onClick={() => setActiveTab(tab.id)} className={`rounded-2xl px-6 py-3 text-sm font-semibold transition ${activeTab === tab.id ? "bg-[#2f3136] text-white shadow-[0_14px_28px_rgba(47,49,54,0.18)]" : "bg-[#f4efe8] text-slate-600 hover:bg-[#ede4d7]"}`}>
                {tab.label} ({tab.count})
              </button>
            ))}
          </div>
          <div className="flex flex-col gap-3 md:flex-row">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products by name, sku, owner..." className="min-w-[280px] rounded-2xl border border-black/10 bg-[#fbfaf8] px-4 py-3 text-[13px] outline-none transition focus:border-[#b56d2d]" />
          </div>
        </div>

        {selectedProducts.length ? (
          <div className="flex flex-wrap gap-2">
            <button className="inline-flex items-center gap-2 rounded-2xl border border-black/10 bg-white px-4 py-2.5 text-[13px] font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50" type="button" onClick={() => runBulkAction("approved")} disabled={Boolean(busyAction)}><Icon name="check" className="h-4 w-4" />Bulk approve</button>
            <button className="inline-flex items-center gap-2 rounded-2xl border border-black/10 bg-white px-4 py-2.5 text-[13px] font-semibold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50" type="button" onClick={() => runBulkAction("rejected")} disabled={Boolean(busyAction)}><Icon name="close" className="h-4 w-4" />Bulk reject</button>
            <button className="inline-flex items-center gap-2 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-[13px] font-semibold text-rose-700 transition hover:bg-rose-100 disabled:cursor-not-allowed disabled:opacity-50" type="button" onClick={() => runBulkAction("delete")} disabled={Boolean(busyAction)}><Icon name="trash" className="h-4 w-4" />Bulk delete</button>
            <button className="inline-flex items-center gap-2 rounded-2xl border border-[#d7c5b1] bg-[#fbf7f1] px-4 py-3 text-[13px] font-semibold text-[#84552b] transition hover:bg-[#f4ece2]" type="button" onClick={exportProducts}><Icon name="download" className="h-4 w-4" />Export</button>
          </div>
        ) : null}
      </div>
                ...
                <tr key={product._id} className="border-b border-[#ece5db] align-top transition hover:bg-[#fcfaf7] last:border-b-0">
                  ...
                  <td className="px-5 py-6 text-ink">
                    <div className="flex min-w-[340px] items-start gap-4">
                      <div className="flex h-18 w-18 shrink-0 items-center justify-center overflow-hidden rounded-[18px] bg-[linear-gradient(135deg,#f5ede2,#ece3d7)]">
                        {productImage ? <img src={productImage} alt={product.name} className="h-full w-full object-cover" /> : <span className="text-xs font-semibold uppercase tracking-[0.22em] text-[#9a6b36]">Item</span>}
                      </div>
                  ...
                  <td className="px-5 py-6 text-slate-900">
                    ...
                    <div className="mt-1 text-xs text-slate-500">
                      {displayMetrics.hasVariants ? "Lowest variant price" : "Current marketplace price"}
                    </div>
                  </td>
                  <td className="px-5 py-6 text-slate-900">
                    <div className="inline-flex min-w-20 items-center justify-center rounded-full bg-[#f3efe8] px-4 py-2 text-[18px] font-semibold text-[#5f4a35]">{displayMetrics.stock}</div>
                  ...
                  <td className="px-5 py-6">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-rose-200 bg-rose-50 text-rose-700 transition hover:bg-rose-100" type="button" onClick={() => removeAdminProduct(product._id)} title="Delete product"><Icon name="trash" /></button>
                    </div>
                  </td>
                </tr>
              );
