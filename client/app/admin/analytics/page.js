...
function buildAnalytics(orders = [], products = [], dateFrom = "", dateTo = "") {
  ...
  const totalSales = filteredOrders.reduce(
    (sum, order) => sum + Math.max(0, Number(order.totalAmount || 0) - Number(order.discountAmount || 0)),
    0
  );
  ...
  filteredOrders.forEach((order) => {
    ...
    (order.items || []).forEach((item) => {
      ...
      const proportionalDiscount = itemsTotal > 0 ? (orderDiscount * originalPrice) / itemsTotal : 0;
      ...
      bucket.totalDiscount += proportionalDiscount;
      ...
      if (hasVariants) {
        ...
        const variantObject = item.variantLabel
          ? { Variant: item.variantLabel, ...optionValues }
          : optionValues;
  ...
  const rows = Array.from(productMap.values()).map((row) => ({
    ...row,
    grossSale: Math.max(0, row.totalSale - row.totalDiscount)
  }));
...
export default function AdminAnalyticsPage() {
  const { token, error, setError } = useAccessToken("Login with an admin account to view analytics.");
  ...
  return (
    <div className="min-w-0 space-y-8">
      <div>
        ...
        <p className="mt-2 text-slate-600">View sales, revenue, and product performance using the same workflow as Custom E Commerce.</p>
      </div>
        ...
        {showDatePicker ? (
          <div className="mt-5 grid gap-6 lg:grid-cols-[220px_1fr]">
            <div className="border-b border-slate-200 pb-4 lg:border-b-0 lg:border-r lg:pb-0 lg:pr-6">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Quick Select</div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-4 2xl:grid-cols-4">
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-semibold uppercase text-slate-500">Total Sales</p>
              <p className="mt-3 text-3xl font-bold text-blue-600">{formatCurrency(analytics.totalSales)}</p>
              <p className="mt-2 text-sm text-slate-500">Net of discounts in selected range</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-semibold uppercase text-slate-500">Total Orders</p>
              <p className="mt-3 text-3xl font-bold text-slate-900">{analytics.totalOrders}</p>
              <p className="mt-2 text-sm text-slate-500">Orders captured in current window</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-semibold uppercase text-slate-500">Total Products</p>
              <p className="mt-3 text-3xl font-bold text-slate-900">{analytics.totalProducts}</p>
              <p className="mt-2 text-sm text-slate-500">Products currently in catalog</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-semibold uppercase text-slate-500">Avg. Order Value</p>
              <p className="mt-3 text-3xl font-bold text-slate-900">{formatCurrency(analytics.avgOrderValue)}</p>
              <p className="mt-2 text-sm text-slate-500">Average revenue per order</p>
            </div>
          </div>
          ...
          <div className="max-w-full overflow-hidden rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            <div className="mb-5 flex flex-wrap items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">Grouped by product with gross sales, order count, and discount distribution.</p>
              </div>
            </div>
              ...
              <table className="w-full min-w-[720px] text-sm md:min-w-[860px]">
                <thead>
                  <tr className="border-b border-slate-200 text-left tracking-[0.18em] text-slate-500">
                    ...
                    <th
                      className="cursor-pointer py-3 px-4 text-right"
                          ...
                          <td className="py-4 pr-4">
                            <button
                              ...
                              className={`text-left font-semibold ${
                                hasVariants ? "text-blue-700 hover:underline" : "text-slate-900"
