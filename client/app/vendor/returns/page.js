    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/55 p-4">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[28px] bg-white p-6 shadow-[0_30px_80px_rgba(15,23,42,0.35)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-700">Return Request</div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-2">
          <div className="space-y-4">
            ...
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Order Info</div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl bg-slate-50 p-4">
              ...
              <select value={draftStatus} onChange={(event) => setDraftStatus(event.target.value)} className="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">
                {STATUS_OPTIONS.map((status) => <option key={status} value={status}>{status}</option>)}
              </select>
              <textarea value={statusNote} onChange={(event) => setStatusNote(event.target.value)} rows={4} placeholder="Status note" className="mt-3 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" />
              <button type="button" onClick={onSave} disabled={saving} className="mt-3 rounded-2xl bg-emerald-700 px-5 py-3 text-sm font-semibold text-white hover:bg-emerald-600 disabled:opacity-60">
...
export default function VendorReturnsPage() {
  const { token, error, setError } = useAccessToken("Login with a vendor account to manage returns.");
  ...
  async function load() {
    ...
    try {
      setLoading(true);
      const response = await marketplaceApi.getVendorReturns(token);
  ...
  async function saveStatus() {
    ...
    try {
      setSaving(true);
      const response = await marketplaceApi.updateVendorReturnStatus(token, selectedReturn._id, { status: draftStatus, statusNote });
  ...
  return (
    <section className="space-y-6">
      <div>
        <div className="text-sm font-semibold uppercase tracking-[0.18em] text-emerald-300">Vendor</div>
        <h1 className="mt-2 text-4xl font-black tracking-[-0.04em] text-white">Returns</h1>
        <p className="mt-2 text-sm text-slate-300">Review requests sent for your products and update their status from the detail modal.</p>
      </div>
      ...
      <div className="rounded-[28px] border border-[#dce5e0] bg-white p-6 shadow-[0_18px_35px_rgba(15,23,42,0.06)]">
        {loading ? (
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={row._id} className="border-b border-slate-100 last:border-b-0">
                    ...
                    <td className="py-4 px-4 text-slate-600">{formatDate(row.orderDate)}</td>
